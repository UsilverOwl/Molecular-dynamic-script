...
#################################################################################################################################################################################################
		echo "1" | $GMX convert-tpr -s md_"$PDB"_prod.tpr -o "RNA.tpr"

		echo "1 1" | $GMX rms -s md_"$PDB"_prod.tpr -f md_"$PDB"_prod.trr -o graph/rmsd.xvg -tu ns
		echo "1 1" | $GMX rms -s RNA.tpr -f RNA.trr -o graph/rmsd2.xvg -tu ns
		echo "1 SOL"| $GMX hbond -s RNA.tpr -f "md_"$PDB"_prod.xtc" -tu ns -num  hbonds_num.xvg -hx hbonds_helix.xvg
 	...
 	#extract frist frame in PDB only for the protein.
		echo "1 1" | $GMX trjconv -s "md_"$PDB"_prod.tpr" -f "md_"$PDB"_clean_temp.xtc" -o "md_"$PDB"_FF.pdb" -pbc mol  -center 1 -b 0 -e 0


				echo "1 1" | $GMX trjconv -s "md_"$PDB"_prod.tpr" -f "md_"$PDB"_clean_temp.xtc" -o "md_"$PDB"_LF1.pdb" -pbc mol  -center 1 -dump -1
...
n_water=$(grep -oP '(?<=SOL\s).*' "topol2.top" | awk '{sum+=$1} END {print sum}')
...
LINE="#include \"dmso.prm\""
	...
	##################################################################################################
		...
		echo "6 6" | $GMX pdb2gmx -f dmso_5%.gro -o $PDB"_processed.gro" -water $WATER  -ignh -ff $FF -ter
...
#################################################################################################################################################################################################
		...
		echo "1 1" | $GMX rms -s md_"$PDB"_prod.tpr -f md_"$PDB"_prod.trr -o graph/rmsd.xvg -tu ns
		echo "1 1" | $GMX rms -s RNA.tpr -f RNA.trr -o graph/rmsd2.xvg -tu ns
		echo "1 DMSO"| $GMX hbond -s RNA.tpr -f "md_"$PDB"_prod.xtc" -tu ns -num  hbonds_num.xvg -hx hbonds_helix.xvg
		echo "1 0" | $GMX trjconv -s "md_"$PDB"_prod.tpr" -f "md_DR4t_prod.xtc" -o "md_"$PDB"_LF1.pdb" -pbc mol  -center 1 -dump -1
		echo "1 0" |$GMX trjconv -pbc mol -f md_DR4t_prod.xtc -o nopbc.xtc -center 1 -s md_DR4t_prod.tpr
 	...
 	#extract frist frame in PDB only for the protein.
		echo "1 1" | $GMX trjconv -s "md_"$PDB"_prod.tpr" -f "md_"$PDB"_clean_temp.xtc" -o "md_"$PDB"_LF2.pdb" -pbc mol  -center 1 -b 0 -e 0
